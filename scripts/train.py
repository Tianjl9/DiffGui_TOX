...
def train(args, config, model, train_iterator, optimizer, scaler, logger, writer, it):
    ...
    batch_toxicity = torch.tensor([float(item) for item in batch.toxicity], device=args.device).unsqueeze(-1)

    batch_lab = torch.cat((batch_sa, batch_qed, batch_aff, batch_toxicity), dim=1)
