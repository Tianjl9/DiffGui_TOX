...
def print_dict(dict_obj, logger):
...
def print_ring_ratio(all_ring_sizes, logger):
    ...
    if total == 0:
        logger.info('No rings found in generated molecules.')
        return
...
def get_dataset_name(config):
    ...
    raise ValueError("Cannot find dataset name from config.dataset or config.data.dataset")
...
if __name__ == '__main__':
    ...
    if num_examples == 0:
        raise FileNotFoundError(
            f"No samples_*.pt files found in config.sample_path: {config.sample_path}"
        )
    ...
    # 毒性字段统计
    n_toxicity_found = 0
    n_toxicity_missing = 0

    # ============================================================
    # Load train / val mols for novelty, uniqueness, diversity
    # ============================================================
    all_ligand_list = []
    ...
    for train_data in tqdm(train_set, desc='Get train mols'):
        ...
        try:
            ...
            if train_ligand_rdmol is not None:
    ...
    for val_data in tqdm(val_set, desc='Get val mols'):
        ...
        try:
            ...
            if val_ligand_rdmol is not None:
    ...
    for example_idx, r_name in enumerate(tqdm(results_fn_list, desc='Eval')):
        ...
        if dataset_name == 'pdbbind':
            pdbid = os.path.basename(r_name).split('.')[0].split('_')[1]
            ligand_ref_path = os.path.join(
                config.protein_root,
                pdbid,
                pdbid + '_ligand.sdf'
            )
        ...
        r = torch.load(r_name, map_location='cpu')
        ...
        try:
            ...
            logger.warning(f"Cannot load reference ligand: {ligand_ref_path}")
        ...
        for sample_idx, mol_info in enumerate(finished_mols):
            ...
            toxicity_score = safe_float(mol_info.get('toxicity', None))

            if toxicity_score is None:
                n_toxicity_missing += 1
            else:
                n_toxicity_found += 1
            ...
            try:
                ...
                if config.verbose:
                    logger.warning(
                        'Evaluation failed for %s, error: %s'
                        % (f'{example_idx}_{sample_idx}', str(e))
                    )
                continue
    ...
    logger.info(
        'Toxicity field found: %d, missing: %d'
        % (n_toxicity_found, n_toxicity_missing)
    )

    # ============================================================
    # Stability / validity
    # ============================================================
    fraction_mol_stable = all_mol_stable / n_success if n_success > 0 else 0
    fraction_atm_stable = all_atom_stable / all_n_atom if all_n_atom > 0 else 0
    fraction_recon = num_samples and (n_success / num_samples) or 0
    fraction_eval = n_success and (n_eval_success / n_success) or 0
    ...
    denom_validity = n_success + n_invalid + n_disconnect
    validity = (
        (n_success + n_disconnect) / denom_validity
        if denom_validity > 0 else 0
    )

    denom_connectivity = n_success + n_disconnect
    connectivity = (
        n_success / denom_connectivity
        if denom_connectivity > 0 else 0
    )
    ...
    sim_dict = scoring_func.get_novelty_and_uniqueness(
        ligand_gen_list,
        all_ligand_list
    )
    ...
    sim_with_ref = scoring_func.get_sim_with_ref(
        ligand_gen_list,
        all_ligand_list
    )
    ...
    if config.eval_mode == 'bond_only':
        c_bond_length_profile = eval_bond_length.get_bond_length_profile(
            all_bond_dist
        )
        ...
        bond_length_dict = eval_all.calc_bonds_JSD(
            ligand_gen_list,
            ligand_ref_list
        )
        angle_dict = eval_all.calc_angles_JSD(
            ligand_gen_list,
            ligand_ref_list
        )
        dihedral_dict = eval_all.calc_dihedrals_JSD(
            ligand_gen_list,
            ligand_ref_list
        )
        ...
        gen_energy_diff, gen_optimized_rmsd = eval_all.get_optimized_RMSD(
            ligand_gen_list
        )

        gen_energy_diff = [
            diff if diff is not None else 0
            for diff in gen_energy_diff
        ]

        logger.info(
            'Generated Mols RMSD with predicte conformations:  '
            'Mean: %.3f Median: %.3f'
            % (np.mean(gen_predicted_rmsd), np.median(gen_predicted_rmsd))
        )

        logger.info(
            'Generated Mols RMSD before and after RDKit optimization:  '
            'Mean: %.3f Median: %.3f'
            % (np.mean(gen_optimized_rmsd), np.median(gen_optimized_rmsd))
        )

        logger.info(
            'Generated Mols energy difference before and after RDKit optimization:  '
            'Mean: %.3f Median: %.3f'
            % (np.mean(gen_energy_diff), np.median(gen_energy_diff))
        )
    ...
    atom_type_js = eval_atom_type.eval_atom_type_distribution(
        success_atom_types
    )
    ...
    toxicity_scores = [
        r['toxicity']
        for r in results
        if r.get('toxicity') is not None
    ]

    if len(qed) > 0:
        logger.info(
            'QED:   Mean: %.3f Median: %.3f'
            % (np.mean(qed), np.median(qed))
        )

    if len(sa) > 0:
        logger.info(
            'SA:    Mean: %.3f Median: %.3f'
            % (np.mean(sa), np.median(sa))
        )

    if len(logp) > 0:
        logger.info(
            'LogP:    Mean: %.3f Median: %.3f'
            % (np.mean(logp), np.median(logp))
        )

    if len(lipinski) > 0:
        logger.info(
            'Lipinski:    Mean: %.3f Median: %.3f'
            % (np.mean(lipinski), np.median(lipinski))
        )

    if len(tpsa) > 0:
        logger.info(
            'TPSA:    Mean: %.3f Median: %.3f'
            % (np.mean(tpsa), np.median(tpsa))
        )

    if len(toxicity_scores) > 0:
        logger.info(
            'Toxicity:  Mean: %.3f Median: %.3f'
            % (np.mean(toxicity_scores), np.median(toxicity_scores))
        )
    else:
        logger.warning(
            'Toxicity: None. No valid toxicity values were found in samples_*.pt.'
        )
    ...
    if config.docking_mode == 'qvina':
        vina = [
            r['vina'][0]['affinity']
            for r in results
            if r['vina'] is not None
        ]

        if len(vina) > 0:
            logger.info(
                'Vina:  Mean: %.3f Median: %.3f'
                % (np.mean(vina), np.median(vina))
            )

    elif config.docking_mode in ['vina_dock', 'vina_score']:
        vina_score_only = [
            r['vina']['score_only'][0]['affinity']
            for r in results
            if r['vina'] is not None and r['vina'].get('score_only') is not None
        ]

        vina_min = [
            r['vina']['minimize'][0]['affinity']
            for r in results
            if r['vina'] is not None and r['vina'].get('minimize') is not None
        ]

        print(f'vina_score_only: {vina_score_only}')
        print(f'vina_min: {vina_min}')

        if len(vina_score_only) > 0:
            logger.info(
        ...
        if config.docking_mode == 'vina_dock':
            vina_dock = [
                r['vina']['dock'][0]['affinity']
                for r in results
                if r['vina'] is not None and r['vina'].get('dock') is not None
            ]
    ...
    # ============================================================
    print_ring_ratio(
        [r['chem_results']['ring_size'] for r in results],
        logger
    )
    ...
    if config.save:
        torch.save({
            ...
            'toxicity_found': n_toxicity_found,
            'toxicity_missing': n_toxicity_missing
