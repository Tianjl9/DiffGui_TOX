...
class ProteinLigandDataset(Dataset):
    ...
    def _process(self):
        ...
        if len(sample) != 11:
            raise ValueError(
                f"Index format error: expected 10 elements (with toxicity), "
                f"but got {len(sample)} elements. "
                f"Please run clean_pdbbind.py with --use-toxicity flag first."
            )

        print(f"Processing {len(index)} items with toxicity data...")
        
        num_skip = 0
        with database.begin(write=True, buffers=True) as db:
            for i, (pocket_fn, _, ligand_fn, _, logp, tpsa, sa, qed, aff, _, toxicity) in enumerate(tqdm(index)):
                if pocket_fn is not None:
                    try:
                        ...
                        num_skip += 1
                        print(f"Skipping {num_skip} {pocket_fn} {ligand_fn}: {e}")
                        continue
                else:
                    num_skip += 1
                    continue
        ...
        success_count = len(index) - num_skip
        print(f"\n{'=' * 50}")
        print(f"Processing Summary:")
        print(f"  Total items: {len(index)}")
        print(f"  Successful: {success_count} ({100 * success_count / len(index):.1f}%)")
        print(f"  Skipped: {num_skip} ({100 * num_skip / len(index):.1f}%)")
        print(f"{'=' * 50}\n")
